        ...
        {faqData.map(({ id, title, content }, index) => {
          return (
            <li key={id} className="group w-full overflow-hidden">
              <label
                htmlFor={id}
                className="cursor-pointer flex items-center justify-between relative heading03B border-b border-b-[#e6e6e6] before:absolute h-30 before:-bottom-px before:h-0.5 before:w-0 before:bg-black before:content-[''] before:transition-[width] before:duration-500 group-has-checked:before:w-full"
              >
                <span className="w-3/4 inline-block truncate">{title}</span>
              ...
              <div className="grid grid-rows-[0fr] overflow-hidden transition-[grid-template-rows] group-has-checked:grid-rows-[1fr]">
                <div className="overflow-hidden">
                  <div className="px-4 py-8 body01M">{content}</div>
                </div>
              </div>
            </li>
          );
